  ...
  <div>
    <div class="box">
      ...
      <div class="field">
        ...
        <div class="control">
          <editor
            api-key="kpj7rgac9iauixcrqhckw6w866lbc50kf0f7ei3xw1k9kseg"
            v-model="NewArticleDescription"
            output-format="html"
            :init="{plugins: ['wordcount lists autoresize'], content_css:'/css/app.css',  toolbar: ['undo redo | formatselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent']}"
          />
        </div>
      </div>
    </div>
    <div class="box">
      ...
      <div v-if="articleTags.length >= 1" class="field">
        ...
        <div>
          <span v-for="(articleTag, index) in articleTags" :key="index" class="content">
            {{articleTag}}
            <a class="delete" @click.once="deleteTag(index)"></a>
          </span>
        </div>
      </div>
      <div v-else>
        ...
        .catch((error) => {
          ...
          if (this.user == "") {
            document.getElementById("errorTitle").innerHTML =
              "You session has run out";
            document.getElementById("errorText").innerHTML =
              "Try logging in again. Refresh to go to login page";
          } else {
            document.getElementById("errorTitle").innerHTML =
              "There is something wrong with the article you are trying to upload";
            document.getElementById("errorText").innerHTML =
              "Check if the image is a jpeg, png, jpg, at a maxium size of 7MB. Check and see if you added a title, and the according tags";
          }
        });
    },
    closeError: function () {
